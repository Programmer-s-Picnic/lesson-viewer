...
const classPolicy = () => ({
  ...
  block_imports: classroomMode,
  allow_imports: classroomMode ? ["math"] : [],
  allow_micropip: !classroomMode,
});

function applyClassroomUI() {
  ui.packagesCard.style.display = classroomMode ? "none" : "block";
  btn.classroom.textContent = classroomMode ? "🎓 Classroom: ON" : "🎓 Classroom";
  toast(classroomMode ? "Classroom Mode enabled" : "Classroom Mode disabled");
}
...
function loadStdin() {
  try {
    ui.stdin.value = localStorage.getItem(STORAGE_STDIN) || "";
  } catch {}
}
function saveStdin() {
  try {
    localStorage.setItem(STORAGE_STDIN, ui.stdin.value || "");
  } catch {}
}

function loadClassroomMode() {
  try {
    return localStorage.getItem(STORAGE_CLASSMODE) === "1";
  } catch {
    return false;
  }
}
function saveClassroomMode() {
  try {
    localStorage.setItem(STORAGE_CLASSMODE, classroomMode ? "1" : "0");
  } catch {}
}
...
function getXP() {
  return parseInt(localStorage.getItem(STORAGE_XP) || "0", 10) || 0;
}
function setXP(v) {
  localStorage.setItem(STORAGE_XP, String(v));
  ui.xp.textContent = String(v);
}
function getStreak() {
  return parseInt(localStorage.getItem(STORAGE_STREAK) || "0", 10) || 0;
}
...
async function runJudge(testList, label) {
  ...
  ui.judgeSummary.innerHTML = `<b>${label}:</b> <span style="font-weight:900">${pass}/${total}</span> • Score: <b>${score}</b> • Time: <span class="pp-dim">${Math.round(
    t1 - t0
  )} ms</span>`;
  ...
  table.innerHTML = `
    <thead>
      <tr><th style="width:120px">Result</th><th>Test</th><th style="width:120px">Status</th></tr>
    </thead>
    <tbody></tbody>`;
  ...
  rows.forEach((r) => {
    ...
    tr.innerHTML = `
      <td><b>${r.ok ? "✅" : "❌"}</b></td>
      <td>
        <div class="pp-dim">${esc(r.name)} ${r.hidden ? "• hidden" : ""}</div>
        <div style="margin-top:6px"><b>Input</b><pre class="pp-mono" style="margin:6px 0 0;padding:8px;border:1px solid var(--pp-border);border-radius:12px;background:rgba(11,18,32,.04)">${esc(
          r.input
        )}</pre></div>
        <div style="margin-top:8px"><b>Expected</b><pre class="pp-mono" style="margin:6px 0 0;padding:8px;border:1px solid var(--pp-border);border-radius:12px;background:rgba(11,18,32,.04)">${esc(
          r.expected
        )}</pre></div>
        <div style="margin-top:8px"><b>Got (stdout)</b><pre class="pp-mono" style="margin:6px 0 0;padding:8px;border:1px solid var(--pp-border);border-radius:12px;background:rgba(11,18,32,.04)">${esc(
          r.got
        )}</pre></div>
        ${
          r.stderr
            ? `<div style="margin-top:8px"><b>stderr</b><pre class="pp-mono" style="margin:6px 0 0;padding:8px;border:1px solid var(--pp-border);border-radius:12px;background:rgba(11,18,32,.04)">${esc(
                r.stderr
              )}</pre></div>`
            : ""
        }
        ${
          r.error
            ? `<div style="margin-top:8px"><b style="color:#b91c1c">Error</b><pre class="pp-mono" style="margin:6px 0 0;padding:8px;border:1px solid #f3c7c7;border-radius:12px;background:rgba(185,28,28,.06)">${esc(
                r.error
              )}</pre></div>`
            : ""
        }
      ...
      <td>${
        r.ok
          ? `<span style="color:var(--pp-ok);font-weight:900">PASS</span>`
          : `<span style="color:var(--pp-bad);font-weight:900">FAIL</span>`
      }</td>`;
...
async function installPkgs() {
  if (classroomMode) return toast("Packages disabled in Classroom Mode");
...
ui.runSamples.onclick = async () => {
  ...
  if (!p) return toast("Pick a problem");
  await runJudge(
    p.examples.map((e) => ({ input: e.input, output: e.output, hidden: false })),
    "Samples"
  );
};
